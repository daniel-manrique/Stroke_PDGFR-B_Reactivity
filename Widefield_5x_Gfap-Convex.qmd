...
title: "Analysis of GFAP covarage (convex hull) of the ischemic hemisphere"
...
**Working dataset**: The `Wide5x_Whole_Gfap_Coverage.csv`data frame containing hemispheric areas measured in FIJI [@schindelin2012] using a custom-made script (https://osf.io/74mqn/files/osfstorage/65aff27499d0100485626ef4) and manual measurements of GFAP convex hull.  
...
Gfap_Convex <- read.csv(file = "ProData/Wide5x_Whole_Gfap_Coverage.csv", header = TRUE)

knitr::kable(Gfap_Convex[1:10, ])
```
...
We use the `brms` package [@brms; @brms-2] to run Bayesian modeling. Here, we use weakly informative priors to optimize the Markov chain Monte Carlo (MCMC) and the exploration of the parameter space. We fit two models to investigate the covarage of GFAP in the ischemic hemisphere. 
...
Gfap_Convex_Fit1 <- 
  brm(
    ...
    file    = "Models/Gfap_Convex/Gfap_Convex_Fit1.rds",
    file_refit = "never") 
...
Gfap_Convex_Fit2 <- 
  brm(
    ...
    seed    = 8807,
    control = list(adapt_delta = 0.99, max_treedepth = 15),
    file    = "Models/Gfap_Convex/Gfap_Convex_Fit2.rds",
    file_refit = "never")
...
We compare the models using WAIC criteria. Please refer to the `Widefield_5x_Gfap-IntDen.qmd` notebook for additional details on this approach. 
...
ggsave(
  ...
  filename = "Plots/Gfap_Convex_DPI.png", 
  width    = 9, 
  height   = 9, 
  units    = "cm")
...
```{r}
#| label: Gfap_Convex?DescribePosterior
#| include: true
#| warning: false
#| message: false
#| results: false
