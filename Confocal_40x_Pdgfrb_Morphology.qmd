...
title: "Analysis of PDGFR-β morphology"
...
Animals were grouped at 0 (sham), 3, 7, 14, and 30 days post-ischemia (DPI). The raw images and pre-processing scripts (if applicable) are available at the Zenodo repository (10.5281/zenodo.10553084) under the name `Confocal_40x_ROIs_CD31-Pdgfrb_CD13.zip`.

**Working dataset**: We employed a python-based analysis pipeline to perform image processing, threshold and cell segmentation (LINK). This generated binary masks with segmented cells that were exported to ilastik [@berg2019] to perform object classification based on the following parameters (as defined by [ilastk](https://www.ilastik.org/documentation/objects/objectfeatures.html)):
...
#install.packages(c("bayesplot", "bayestestR", "brms","dplyr", "easystats", "emmeans", "FactoMineR", "factoextra", "ggcorrplot", "ggplot", "modelr", "patchwork", "poorman", "tidybayes", "tidyverse", "viridis"))
...
folder_path <- "Data_Raw/Confocal_40x_Pdgfrb_Morphology"
...
write.csv(Pdgfrb_Morphology, "Data_Processed/Confocal_40x_Pdgfrb_Morphology_40x.csv", row.names = FALSE)
```
...
Pdgfrb_Morphology <- read.csv(file = 'Data_Processed/Confocal_40x_Pdgfrb_Morphology_40x.csv', header = TRUE)
...
ggsave(
  ...
  filename = "plots/Confocal_40x_Pdgfrb_MorphologicalAnalysis/Pdgfrb_Morphology_corr_fig.png", 
  width    = 12, 
  height   = 10, 
  units    = "cm")
...
After, we perform the principal component analysis using the `princomp` function, using the correlation matrix stored as `Pdgfr_Morphology_corr`. We print the numerical and graphical summaries of this step
...
ggsave(
  ...
  filename = "plots/Confocal_40x_Pdgfrb_MorphologicalAnalysis/Pdgfrb_Morphology_Biplot_fig.png", 
  width    = 11, 
  height   = 9, 
  units    = "cm")
...
Pdgfrb_Morphology_Spyder_Sum <- Pdgfrb_Morphology_Spyder %>%
  ...
  reshape_longer(c("Area", "ConvexHull",  "BranchLenght", "Intensity_Mean"))
...
ggsave(
  ...
  filename = "plots/Confocal_40x_Pdgfrb_MorphologicalAnalysis/Pdgfrb_Morphology_Spyder_fig.png", 
  width    = 9, 
  height   = 15, 
  units    = "cm")
...
Pdgfrb_Morphology_Fit1 <- 
  brm(
    ...
    file    = "Models/Pdgfrb_Morphology/Pdgfrb_Morphology_Fit1.rds",
    file_refit = "never") 
```
...
```{r}
#| label: fig-Psgfrb_Morphology_CE
#| include: true
#| warning: false
#| message: false
#| fig-cap: Posterior predictive checks for PDGFR-β+ morphology
...
ggsave(
  ...
  filename = "Plots/Confocal_40x_Pdgfrb_MorphologicalAnalysis/Pdgfrb_Morphology_Convex_fig.png", 
  width    = 12, 
  height   = 10, 
  units    = "cm")
...
ggsave(
  ...
  filename = "Plots/Confocal_40x_Pdgfrb_MorphologicalAnalysis/Pdgfrb_Morphology_Branch_fig.png", 
  width    = 12, 
  height   = 10, 
  units    = "cm")
...
ggsave(
  ...
  filename = "Plots/Confocal_40x_Pdgfrb_MorphologicalAnalysis/Pdgfrb_Morphology_Inten_fig.png", 
  width    = 12, 
  height   = 10, 
  units    = "cm")
