...
title: "Analysis of Klf4 expression in defined ROIs"
...
This notebook reports the analysis of KLF4 expression in defined ROIs (injured cortex and contralateral hemisphere following cerebral ischemia).

**Parent dataset:** KLF4, PDGFR-β, and CD31 stained ischemic hemispheres imaged at 20x in a confocal microscope. Samples are grouped at 3, 7, 14, and 30 days post-ischemia (DPI). The raw images and pre-processing scripts (if applicable) are available at the Zenodo repository (10.5281/zenodo.10553084) under the name XXXXXXXXXXX

**Working dataset**: The `Data_Raw/Confocal_20x_Pdgfrb-Klf4_Coloc/Confocal_20x_ROIs_Pdgfrb-Klf4_Image.csv`data frame containing the raw output from CellProfiller [@stirling2021]. The CellProfiller pipeline used to perform the KLF4+ cell detection is available at XXXX

We perform scientific inference based on the number of colocalized KLF4 and PDGFR-β onjects.
...
We load the `Confocal_20x_ROIs_Pdgfrb-Klf4_Image.csv` dataset containing summary information about KLF4 and PDGFR-β colocalized objects.
...
Klf4_Cells <- read.csv(file = "Data_Raw/Confocal_20x_Pdgfrb-Klf4_Coloc/Confocal_20x_ROIs_Pdgfrb-Klf4_Image.csv", header = TRUE)
...
write.csv(Klf4_Coloc_Data, "Data_Processed/Conf20x_ROIs_Klf4-Pdgfrb_Coloc.csv", row.names = FALSE)
```

With the data handled, we proceed to exploratory data visualization to appreciate the tendency of the data. We focus on the KLF4+ object detections. 
...
-   **Klf4_Coloc_Mdl1:** We use `DPI` to calculate the probability of colocalization in contralateral regions using a logistic function:
...
\text{Coloc} | \text{trials}(Klf4) \sim \text{Binomial}(Klf4, p) \\
\text{logit}(p) = \beta_0 + \beta_{DPI}[DPI] + \beta_{Region}[Region] + \beta_{DPI \times Region}[DPI \times Region]
...
Klf4_Coloc_Fit1 <- 
  brm(
    ...
    file    = "Models/Klf4_Expression/Klf4_Coloc_Fit1.rds",
    file_refit = "never") 
...
Klf4_Coloc_Fit2 <- 
  brm(
    ...
    seed    = 8807,
    control = list(adapt_delta = 0.99, max_treedepth = 15),
    file    = "Models/Klf4_Expression/Klf4_Coloc_Fit2.rds",
    file_refit = "never") 
...
Klf4_Coloc_Fit3 <- 
  brm(
    ...
    seed    = 8807,
    control = list(adapt_delta = 0.99, max_treedepth = 15),
    file    = "Models/Klf4_Expression/Klf4_Coloc_Fit3.rds",
    file_refit = "never") 
...
ggsave(
  ...
  filename = "Plots/Klf4_Coloc_Fig.png", 
  width    = 10, 
  height   = 8, 
  units    = "cm")
...
Pdgfrb_Coloc_Mdl1 <- bf(Coloc | trials(Pdgfrb) ~ DPI * Region)

get_prior(Pdgfrb_Coloc_Mdl1, Klf4_Coloc_Regions, family = binomial())

Pdgfrb_Coloc_Mdl1_Prior <- prior(beta(2, 10), class = b, lb = 0, ub = 1)
...
Pdgfrb_Coloc_Fit1 <- 
  brm(
    data    = Klf4_Coloc_Regions,
    family  = binomial(), 
    formula = Pdgfrb_Coloc_Mdl1,
    prior   = Pdgfrb_Coloc_Mdl1_Prior,
    chains  = 4,
    cores   = 4,
    warmup  = 2500, 
    iter    = 5000, 
    seed    = 8807,
    control = list(adapt_delta = 0.99, max_treedepth = 15),
    file    = "Models/Klf4_Expression/Pdgfrb_Coloc_Fit1.rds",
    file_refit = "never") 
                     
# Add loo for model comparison
Pdgfrb_Coloc_Fit1 <- 
  add_criterion(Pdgfrb_Coloc_Fit1, c("loo", "waic", "bayes_R2"))
```
## Visualization of posterior distributions
...
# Model 1
Pdgfrb_Coloc_Fit1_CE <- 
  conditional_effects(Pdgfrb_Coloc_Fit1)

Pdgfrb_Coloc_Fit1_CE <- plot(Pdgfrb_Coloc_Fit1_CE, 
       plot = FALSE)[[3]]

Pdgfrb_Coloc_Fit1_fig <- Pdgfrb_Coloc_Fit1_CE  + 
  scale_y_continuous(name = expression ("(p) PDGFR-β/KLF4"),
                     limits = c(0.15, 0.4),
...
ggsave(
  plot     = Pdgfrb_Coloc_Fit1_fig, 
  filename = "Plots/Pdgfrb_Coloc_Fig.png", 
  width    = 10, 
  height   = 8, 
  units    = "cm")

Pdgfrb_Coloc_Fit1_fig 
```
...
We can calculate the contrast to obtain precise estimates (TO DO). Numbers do not make sense.
```{r}
# We generate a data frame with the contrast
Pdgfrb_Coloc_Contrast <- Pdgfrb_Coloc_Fit1%>%
emmeans(~ DPI + Region, type = "response") %>%
contrast(method = "revpairwise") %>%
gather_emmeans_draws() %>% sample_n(100)


# Subset relevant data fo the contrast graph
Pdgfrb_Coloc_Contrast_Sub <- Pdgfrb_Coloc_Contrast[
  (Pdgfrb_Coloc_Contrast$contrast=="7D Peri - 7D Contra"|
   Pdgfrb_Coloc_Contrast$contrast=="7D Ipsi - 7D Contra"|
   Pdgfrb_Coloc_Contrast$contrast=="14D Peri - 14D Contra"|
   Pdgfrb_Coloc_Contrast$contrast=="14D Ipsi - 14D Contra"|
   Pdgfrb_Coloc_Contrast$contrast=="30D Peri - 30D Contra"| 
   Pdgfrb_Coloc_Contrast$contrast=="30D Peri - 30D Contra"),]

Pdgfrb_Coloc_Contrast_Sub$contrast <- 
  factor(Pdgfrb_Coloc_Contrast_Sub$contrast, 
         levels = c("7D Peri - 7D Contra", 
                    "7D Ipsi - 7D Contra", 
                    "14D Peri - 14D Contra", 
                    "14D Ipsi - 14D Contra", 
                    "30D Peri - 30D Contra",
...
Pdgfrb_Coloc_Contras_fig <- ggplot(Pdgfrb_Coloc_Contrast_Sub , 
