...
First, we'll model the total number of Ki67/PDGFR-β colocalized cells as a support to interpret a second model investigation the proportion of perivascular PDGFR-β+ cells colocalized with Ki67. In this case, we use a negative binomial distribution for count data accounting for overdispersion. The mdel takes the following notation:
...
To facilitate the regression and its interpretation, we exclude the cases wichh do not have any PDGFR-β/Ki67 colocalization.
...
Ki67_Fit2 <- 
  brm(
    ...
    seed    = 8807,
    control = list(adapt_delta = 0.99, max_treedepth = 15),
    file    = "Models/Ki67_Pdgfrb_Coloc/Ki67_Pdgfrb_ROIs_Fit2.rds",
    file_refit = "never") 
...
Ki67_Pdgfrb_fig <- Ki67_Pdgfrb_DPI + 
  ...
  filename = "Plots/Confocal_20x_Ki67-Pdgfrb_Coloc/Ki67-Pdgfrb_Coloc.png", 
  width    = 9, 
  height   = 9, 
  units    = "cm")
...
Ki67_Pdgfrb_Vascular_fig <- Ki67_Pdgfrb_Vascular + 
  ...
  filename = "Plots/Confocal_20x_Ki67-Pdgfrb_Coloc/Ki67-Pdgfrb_Vascular_Coloc.png", 
  width    = 9, 
  height   = 9, 
  units    = "cm")
...
describe_posterior(
  ...
  centrality = "median")
```
The tables show the regression estimates. For the first model, estimating the total number of Ki67/PDGFR-β colocalized cells, the output is shown in the log scale. For the second model, estimating the proportion of parenchymal PDGFR-β/Ki67+ cells, the ourput is sown on the log odds scale.  
