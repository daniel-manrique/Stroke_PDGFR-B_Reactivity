...
title: "Point pattern analysis (PPA) of PDGFR-β and GFAP (widefield 5x) - Striatal lesions"
...
In this notebook we analize the covariance between PDGFR-β and GFAP cells in brain with striatal injuty using function from the `spatstat` package [@spatstat; @baddeley2005]. First, we load the point patterns:
...
PointPatterns_Str <- readRDS("PointPatterns/Widefield_5x_Ipsilateral_Pdgfr-Gfap_Str_PointPatterns.rds")

# We assign the animal ID to row names
PointPatterns_Str <- `row.names<-.hyperframe`(PointPatterns, PointPatterns$ID)
...
We generate density kernels and add them to the hyperframe. For additional details on this procedure, please refer the the `Widefield_5x_Pdgfrb-Gfap_Covariance` notebook processing the data from cortico-striatal lesions. 

```{r}
#| label: PPP_Pdgfrb_Str_Density
#| include: true
#| warning: false
#| message: false

PointPatterns_Str$Pdgfr_Low_Density <- with (PointPatterns, density(Pdgfr_Low, sigma = 0.2))

PointPatterns_Str$Pdgfr_High_Density <- with (PointPatterns, density(Pdgfr_High, sigma = 0.2))

PointPatterns_Str$Gfap_Density <- with (PointPatterns, density(Gfap, sigma = 0.2))
```
...
Pdgfr_Low_Colmap <- colourmap(topo.colors(256), range = c(0, 200))
Pdgfr_High_Colmap <- colourmap(topo.colors(256), range = c(0, 200))

par(mfrow = c(2,5), mar=c(1,1,1,1), oma=c(1,1,1,1))

## For PDGFR_Low

plot(PointPatterns_Str$Pdgfr_Low_Density$Td94, col = Pdgfr_Low_Colmap, main = "14 DPI")  
plot(PointPatterns_Str$Pdgfr_Low_Density$Td102, col = Pdgfr_Low_Colmap, main = "30 DPI")  

## For PDGFR_High

plot(PointPatterns_Str$Pdgfr_High_Density$Td94, col = Pdgfr_High_Colmap, main = "")
plot(PointPatterns_Str$Pdgfr_High_Density$Td102, col = Pdgfr_High_Colmap, main = "")  
```
...
## For 14D
Pdgfrb_Str_Rhohat_14D <- with(PointPatterns_Str_14D, spatstat.explore::rhohat.ppp(Pdgfr_High, Gfap_Density, do.CI = FALSE))
Pdgfrb_Str_Rhohat_14D <- pool(Pdgfrb_Str_Rhohat_14D)

## For 30D
Pdgfrb_Str_Rhohat_30D <- with(PointPatterns_Str_30D, spatstat.explore::rhohat.ppp(Pdgfr_High, Gfap_Density, do.CI = FALSE))
...
png("Plots/Pdgfrb_Str_Rhohat_14D.png", width = 700, height = 500)
par(mar=c(7,10,2,2))
...
png("Plots/Pdgfrb_Str_Rhohat_30D.png", width = 700, height = 500)
par(mar=c(7,10,2,2))
...
Pdgfrb_Mdl1 <- mppm(Pdgfr_High ~ DPI/Gfap_Density, data = PointPatterns_Str)
