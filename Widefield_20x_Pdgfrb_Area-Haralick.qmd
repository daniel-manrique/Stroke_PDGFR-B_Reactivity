...
**Working dataset**: The `Data_Raw/Widefield_20x_ROIs_Pdgfrb_Cells/Image.csv`contains the quantification of the area covered by the detected cells after processing using CellProfiller. The pipeline is available at https://osf.io/k9zbe. Also, we load the `Data_Raw/Raw_Widefield_20x_ROIs_Pdgfrb_Haralick.csv` containing the computed haralick features using this [python code](https://github.com/elalilab/Stroke_PDGFR-B_Reactivity/blob/main/Widefield_20x_ROIs_Pdgfrb_HaralickComputation.ipynb).
...
# Analysis of Haralick features for PDGFR-β (Tecture analysis)

## Load the dataset

We load and handle the `Data_Raw/Raw_Widefield_20x_ROIs_Pdgfrb_Haralick.csv` dataset containing the Haralick features calculated in Python. Here, We subset the data and make it usable.
...
# Load the data
Pdgfrb_Haralick <- read.csv(file = 'Data_Raw/Raw_Widefield_20x_ROIs_Pdgfrb_Haralick.csv', header = TRUE)
...
knitr::kable(Pdgfrb_Haralick[1:10, ])
...
Next, we perform principal component analysis (PCA) as a dimensionality reduction method to for multiple variablesv. We will use the `factoMineR`[@FactoMineR] and `factoextra` [@factoextra] packages to have access to the PCA and visualization functions.
...
ggsave(
  ...
  filename = "Plots/Widefield_20x_ROIs_Pdgfrb_Area-Haralick/Pdgfrb_Haralick_corr _fig.png", 
  width    = 12, 
  height   = 10, 
  units    = "cm")
...
ggsave(
  ...
  filename = "Plots/Widefield_20x_ROIs_Pdgfrb_Area-Haralick/Pdgfrb_Haralick_Biplot_fig.png", 
  width    = 12, 
  height   = 12, 
  units    = "cm")
...
```
Altogether, the PCA analysis suggest that all the variables have a similar loading for the coponents. This means, we cannot detect more relevant features. Given that we dealing with images that change in intensity, distribution, and density in the course of injury, we judge that the variations in the texture, uniformity, and the complexity of the patterns are features of interest. Based on this, we decided to rely in the following features:
...
**Inverse Difference Moment (IDM) / Homogeneity:** Given our interest in the distribution and density of cells, homogeneity reveals cells cluster or dispersion during the injurt course.
...
ggsave(
  ...
  filename = "Plots/Widefield_20x_ROIs_Pdgfrb_Area-Haralick/Pdgfrb_Haralick_Spyder_fig.png", 
  width    = 9, 
  height   = 15, 
  units    = "cm")
