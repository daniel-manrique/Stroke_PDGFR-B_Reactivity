...
Ki67_Cells <- read.csv(file = "Data_Raw/Widefield_10x_Pdgfrb-Ki67_Coloc/Image.csv", header = TRUE)
...
write.csv(Ki67_Data, "Data_Processed/Widefield_10x_Ipsilateral_Ki67-Pdgfrb_Coloc.csv", row.names = FALSE)
```
...
This model uses the same priors for model 1. The election of this weakly informative priors is based on previous research XXXX suggesting that up to 50% of PDGFR-β+ cells proliferate in the subsequent days following ischemia.
...
Ki67_Pdgfrb_Fit1 <- 
  brm(
    ...
    file    = "Models/Ki67_Pdgfrb_Coloc/Ki67_Pdgfrb_Fit1.rds",
    file_refit = "never") 
...
Ki67_Pdgfrb_Fit2 <- 
  brm(
    ...
    seed    = 8807,
    control = list(adapt_delta = 0.99, max_treedepth = 15),
    file    = "Models/Ki67_Pdgfrb_Coloc/Ki67_Pdgfrb_Fit2.rds",
    file_refit = "never") 
