...
As we want to evaluate the effects of recombination in each time point, we fit a linear model using  a Poisson distribution. Here, we fit the model for the total number of parenchymal PDGFR-β cells conditioning on the interaction between DPI and condition (sham or MCAO).The model takes the following notation:
