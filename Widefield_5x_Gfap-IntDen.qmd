...
title: "Analysis of GFAP expression (Integrated density)"
...
csl: science.csl
bibliography: Ref_Shrinkage.bib
---
...
**Parent dataset:** PDGFR-β and GFAP-stained ischemic hemispheres imaged at 5x (with stitching). Samples are grouped at 0 (Sham), 3, 7, 14, and 30 days post-ischemia (DPI). The raw images and pre-processing scripts (if applicable) for are available at the Zenodo repository (10.5281/zenodo.10553084) under the name `Widefield_5x_Gfap-Pdgfrb.zip`.

**Working dataset**: The `Pdgfrb_Gfap_Ipsi_5x_Inten.csv`data frame containing the integrated density of GFAP measured in the ipsilateral hemisphere (`Gfap_IntDen`) using FIJI [@schindelin2012]. We perform scientific inference on this variable as a basis to understand astrocyte reactivity in our model. The current knowledge suggest a close to linear increase in GFAP expression from the injury onset.
...
Gfap_Summary <- read.csv(file = "ProData/Wide5x_Ipsi_Pdgfrb_Gfap_Inten.csv", header = TRUE)

## Keep relevant columns
Gfap_Summary  <- subset(Gfap_Summary, select = c("MouseID", "DPI", "Condition", "Tissue_Area", "Gfap_IntDen"))

knitr::kable(Gfap_Summary[1:10, ])
```

Our variable of interest `Gfap_IntDen` is the product of the stained area and the mean intensity as calculated in FIJI using the script available at XXXXXX. In this case, the units can be considered arbitrary and its interpretation is useful only relative to others. Knowing the GFAP dynamics following injury, we expect a progressive increase of this marker. We also keep the `Tissue_Area` variable to fir a model relating the two phenomena. 
...
We use the `brms` package [@brms; @brms-2] to run Bayesian modeling. We employ weakly informative priors in all the cases to optimize the Markov chain Monte Carlo (MCMC) and the exploration of the parameter space.
...
$$
GFAP-IntDen_{i} = \alpha + \beta_{1} DPI_{i} + \epsilon_{i}
$$
...
$$ 
 Gfap\_IntDen = \beta_0 + \sum_{i=1}^{k_1} \sum_{j=1}^{k_2} \beta_{ij} B_{1i}(DPI) B_{2j}(Tissue\_Area) + \epsilon
$$ 
...
## Fit the models for GFAP reactivity
...
Gfap_IntDen_Fit1 <- 
  brm(
    ...
    file    = "Models/GfapReactivity/Gfap_IntDen_Fit1.rds",
    file_refit = "never") 
...
Gfap_IntDen_Fit2 <- 
  brm(
    ...
    seed    = 8807,
    control = list(adapt_delta = 0.99, max_treedepth = 15),
    file    = "Models/GfapReactivity/Gfap_IntDen_Fit2.rds",
    file_refit = "never") 
...
Gfap_IntDen_Fit3 <- 
  brm(
    ...
    file    = "Models/GfapReactivity/Gfap_IntDen_Fit3.rds",
    file_refit = "never") 
...
Gfap_IntDen_Fit4 <- 
  brm(
    ...
    seed    = 8807,
    control = list(adapt_delta = 0.99, max_treedepth = 15),
    file    = "Models/GfapReactivity/Gfap_IntDen_Fit4.rds",
    file_refit = "never") 
...
$$
In our case, we use the function to visualize the effect of DPI on GFAP expression and the estimated sigma.
```{r}
...
We begin by evaluating the sample predictions using `pp_check` for model 3 (Mdl4). In the graph, 𝘺 shows the data and y\~ the simulated data.
